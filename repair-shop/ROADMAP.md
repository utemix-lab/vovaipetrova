...
> **Компактная карта:** [`STATUS.md`](../STATUS.md) — главная точка входа в текущее состояние системы.
...
| P6.0d — LLM Integration | Отложено |
| P6.0e — UI Projection | Отложено |

---

## P7.0 — Declarative Layer (Track 2 + Track 3 + Track 4)
...
Текущая архитектура:
- **Граф** — декларативный (JSON)
- **Проекции** — код (JavaScript)
- **Механики** — код (JavaScript)
- **UI** — код (HTML/CSS/JS)

LLM может работать только с декларативными данными. Код — "чёрный ящик".

### Цель

Сделать проекции, механики и UI декларативными:

```
┌─────────────────────────────────────────────────────────────┐
│  Декларативный слой (JSON/YAML)                             │
│  - projections.json — правила трансформации графа           │
│  - operators.json — триггеры и действия                     │
│  - ui-config.json — компоненты и layout                     │
└─────────────────────────────────────────────────────────────┘
                          ↓
┌─────────────────────────────────────────────────────────────┐
│  Интерпретаторы (код, неизменяемый)                         │
│  - ProjectionEngine                                         │
│  - OperatorEngine                                           │
│  - UIEngine                                                 │
└─────────────────────────────────────────────────────────────┘
...
|-------------|----------|
| **LLM-редактирование** | LLM может создавать/изменять проекции, механики, UI |
| **No-code** | Пользователь описывает, что хочет — система генерирует JSON |
| **Версионирование** | Проекции и механики — данные, можно хранить историю |
| **Портируемость** | Конфигурация переносится между инстансами |

### Этапы

| Этап | Описание | Статус |
|------|----------|--------|
| P7.0a | Declarative Projections | Ожидает |
| P7.0b | Declarative Operators | Ожидает |
| P7.0c | Declarative UI | Ожидает |
| P7.0d | LLM Integration | Ожидает |

### Зависимости

- Требует стабильного P6.0 (спецификация)
- Требует анализа существующих проекций и механик
- Требует DSL-дизайна

### Стратегический вопрос

**Это большая задача.** Альтернативы:
1. Полный DSL — максимальная гибкость, максимальная сложность
2. Шаблоны + параметры — ограниченная гибкость, проще реализовать
3. Визуальный редактор — no-code, но требует UI-разработки
4. Гибридный подход — базовые операции декларативны, сложные — код

**Решение отложено до стратегического анализа.**
...
## Стратегический анализ: Ценность Платформы

**Дата:** 13 февраля 2026

### Честный вопрос

> В чём ценность Платформы? Что она даёт пользователю, чего не даёт Obsidian?

### Текущее состояние

| Obsidian | MeaningEngine |
|----------|---------------|
| Готовый продукт | Фреймворк |
| Плагины из магазина | Требует разработки |
| Markdown + Graph View | Типизированный граф |
| Миллионы пользователей | 1 пользователь |

**Честно:** Сейчас MeaningEngine — это **инструмент для разработчика**, не для конечного пользователя.

### Потенциальные преимущества

| Преимущество | Описание | Реализовано? |
|--------------|----------|--------------|
| **Типизация** | Граф с валидацией, не просто заметки | ✓ Да |
| **Инварианты** | Гарантии целостности | ✓ Да |
| **Multi-world** | Разные миры на одном движке | ✓ Да |
| **LLM-интеграция** | Структурированный контекст | Частично |
| **Декларативность** | No-code создание | ✗ Нет |
| **UI** | Визуальный интерфейс | ✗ Только dev |
...
1. **Декларативный слой (P7.0)** — LLM редактирует JSON, не код
2. **Hosted Platform** — веб-сервис, пользователь не видит код
3. **Визуальный редактор** — drag-and-drop вместо кода
4. **MCP/Tool Use** — LLM вызывает API напрямую

### Кто целевой пользователь?

| Пользователь | Что нужно | Подходит? |
|--------------|-----------|-----------|
| **Разработчик** | Фреймворк для своего проекта | ✓ Да |
| **Автор/Исследователь** | Инструмент для работы с идеями | ? Частично |
| **Обычный пользователь** | Готовый продукт | ✗ Нет |

### Варианты развития

#### Вариант A: Фреймворк для разработчиков
...
- Ценность: готовая архитектура для семантических графов
- Конкуренты: Neo4j, GraphQL, custom solutions

#### Вариант B: Hosted Platform

```
https://meaningengine.app
```

- Целевая аудитория: авторы, исследователи
- Ценность: работа с графом без кода
- Требует: веб-интерфейс, хостинг, аутентификация

#### Вариант C: Obsidian Plugin

```
Obsidian → MeaningEngine Plugin
```

- Целевая аудитория: пользователи Obsidian
- Ценность: типизация и валидация поверх Obsidian
- Требует: интеграция с Obsidian API

#### Вариант D: Личный инструмент

```
VovaIPetrova = единственный мир
```

- Целевая аудитория: Автор
- Ценность: рефлексивный инструмент для себя
- Не требует масштабирования

### Вывод

**Сейчас Платформа — это архитектурный эксперимент.**

Она доказала:
- Граф можно типизировать
- Мир можно отделить от движка
- Спецификацию можно формализовать

Она не доказала:
- Что это нужно кому-то кроме Автора
- Что LLM-интеграция даёт реальную ценность
- Что декларативный слой реализуем

### Рекомендация

Прежде чем строить P7.0, ответить на вопрос:

**Для кого это?**

1. Если для себя — VovaIPetrova достаточно
2. Если для разработчиков — нужна документация и примеры
3. Если для пользователей — нужен hosted product

**Решение за Автором.**

---

## P8.0 — Character Cabins (Track 1 + Track 3 + Track 4)
...
```
Персонаж = Линза на Внешний Мир + Псевдо-функция во Внутреннем Мире
```

Каждый персонаж:
1. **Линза компетенции** — фильтрует знания Внешнего Мира через свою экспертизу
2. **Псевдо-функция** — имеет метафорическую связь с модулем Платформы
3. **Кабина** — интерфейс, где видна его "механика"

### Двойная природа персонажа

```
┌─────────────────────────────────────────────────────────────┐
│  ВНЕШНИЙ МИР (знания, ресурсы, ссылки)                      │
│  ├── Кодинг, IDE-агенты, архитектура                        │
│  ├── Музыка, теория, практика                               │
│  ├── Философия, концепции, тексты                           │
│  └── ...                                                    │
└─────────────────────────────────────────────────────────────┘
              ↓ (линза компетенции)
┌─────────────────────────────────────────────────────────────┐
│  ПЕРСОНАЖ                                                   │
│  ├── Руна: видит кодинг, IDE, архитектуру                   │
│  ├── Вова: видит музыку, творчество                         │
│  ├── Петрова: видит философию, рефлексию                    │
│  └── ...                                                    │
└─────────────────────────────────────────────────────────────┘
              ↓ (псевдо-функция)
┌─────────────────────────────────────────────────────────────┐
│  ВНУТРЕННИЙ МИР (Платформа, Cryptocosm)                     │
│  ├── Руна → SpecificationReader                             │
│  ├── Вова → ???                                             │
│  ├── Петрова → ???                                          │
│  └── ...                                                    │
└─────────────────────────────────────────────────────────────┘
...
| Слой | Что видит/делает |
|------|------------------|
| **Внешний Мир** | Кодинг, IDE-агенты, архитектура систем |
| **Публичное лицо** | Страница персонажа, нарратив, связи в графе |
| **Workbench** | Где она создаёт код, артефакты |
| **Кабина** | Где видно, КАК она работает |
| **Псевдо-функция** | SpecificationReader — "спроси Руну о спецификации" |

### Живой ландшафт

**Не симметричный, не выровненный:**

| Персонаж | Компетенция | Глубина связи с Платформой | Игровая роль |
|----------|-------------|---------------------------|--------------|
| Руна | Кодинг | Глубокая (SpecificationReader) | Техническая |
| Вова | Музыка | Поверхностная? | Творческая |
| Петрова | Философия | Метафорическая? | Рефлексивная |
| Автор | Всё | Полная (extended-mind) | Демиург |

**Асимметрия создаёт игру:**
- Руна знает архитектуру, но не понимает музыку
- Вова чувствует гармонию, но не видит код
- Петрова задаёт вопросы, но не даёт ответов
- Автор видит всё, но действует через персонажей

### Кабина как интерфейс

```
┌─────────────────────────────────────────────────────────────┐
│  КАБИНА РУНЫ                                                │
│  ├── Граф (тот же, что в Universe, но с фильтром)           │
│  ├── Чат с LLM (через линзу Руны)                           │
│  ├── RAG (документы по компетенции Руны)                    │
│  ├── Workbench (инструменты Руны)                           │
│  └── Связь с Платформой (SpecificationReader)               │
└─────────────────────────────────────────────────────────────┘
```

**Кабина — это:**
- Проекция графа через линзу персонажа
- Интерфейс взаимодействия с LLM
- Демонстрация "внутреннего мира" персонажа
- Игровой артефакт (можно показать в Universe)

### Архитектура подыгрывает

Платформа не сковывает, а **усиливает** игру:

| Архитектурный элемент | Игровое использование |
|-----------------------|-----------------------|
| `SpecificationReader` | "Руна объясняет спецификацию" |
| `GraphValidator` | "??? проверяет целостность" |
| `ChangeProtocol` | "??? предлагает изменения" |
| `LLMReflectionEngine` | "??? рефлексирует о системе" |

### Этапы

| Этап | Описание | Статус |
|------|----------|--------|
| P8.0a | Концепция Кабин (этот документ) | ✓ Концептуализация |
| P8.0b | Типы узлов: Cabin, Workbench, Lens | Ожидает |
| P8.0c | Связь персонаж → модуль Платформы | Ожидает |
| P8.0d | UI Кабины (проекция графа + чат) | Ожидает |
| P8.0e | RAG по компетенции персонажа | Ожидает |
| P8.0f | Демонстрация Кабины в Universe | Ожидает |

### Зависимости

- P7.0 (Declarative Layer) — для декларативного описания Кабин
- P6.0 (Specification) — для связи персонаж → модуль
- RAG-инфраструктура — для семантики Внутреннего Мира

### RAG: Внутренний Мир, не Внешний

**Важное уточнение:**

| Источник знаний | Механизм доступа |
|-----------------|------------------|
| **Внешний Мир** | Pointer-теги (ссылки на ресурсы) |
| **Внутренний Мир** | RAG (семантический поиск) |

**Почему RAG для Внутреннего Мира:**
- LLM не удерживает контекст даже небольшого Мира
- Граф, нарратив, спецификация — слишком много для чата
- RAG даёт структурированный доступ к данным

**Что индексировать:**

| Источник | Содержимое |
|----------|------------|
| `universe.json` | Узлы, рёбра, метаданные |
| `schema.json` | Типы, описания |
| `specification.json` | Контракты, методы |
| Нарратив | Тексты страниц персонажей |
| ROADMAP, ONTOLOGY | Архитектурные решения |

**Pointer-теги vs RAG:**
- Pointer-теги — для навигации человека во Внешний Мир
- RAG — для LLM, чтобы понимать Внутренний Мир

Они дополняют друг друга, не заменяют.

### Ключевой вопрос

**Как распределить модули Платформы между персонажами?**

Это не техническое решение — это **драматургическое**. Каждая связь должна быть осмысленной:
- Почему Руна отвечает за спецификацию?
- Кто отвечает за валидацию?
- Кто отвечает за рефлексию?
...
| Этап | Статус |
|------|--------|
| P8.0a — Концепция Кабин | ✓ Концептуализация |
| P8.0b — Типы узлов | Ожидает |
| P8.0c — Связь персонаж → модуль | Ожидает |
| P8.0d — UI Кабины | Ожидает |
| P8.0e — RAG по компетенции | Ожидает |
| P8.0f — Демонстрация в Universe | Ожидает |

---

## Инструменты (Instruments) — уточнение
...
```
Инструменты ≠ Онтология
Инструменты = UI-виджеты другого типа
```
...
|-----------|---------|----------|
| **Практики** | "Режиссура UI", "Осмысление области" | Линзы/методы |
| **Возможности** | "Создать обзор", "Написать туториал" | Действия |
| **Каталоги** | Справочники с pointer-тегами | Навигация во Внешний Мир |
| **Режимы** | Query Mode | Контексты взаимодействия |

### Ключевые решения

1. **Не узлы графа** — не часть онтологии
2. **UI-виджеты** — вызываются так же, как навигационные, но другого типа
3. **Унификация** — не наплодили элементов, а унифицировали навигацию
4. **Платформа не знает** — это уровень UI, не архитектуры
5. **LLM объясним** — как решим, так и объясним

### Визуальная картина

```
┌─────────────────────────────────────────────────────────────┐
│  Universe (публичный граф)                                  │
│  ├── Навигационные виджеты (узлы графа)                     │
│  └── Инструментальные виджеты (Практики, Возможности...)    │
└─────────────────────────────────────────────────────────────┘
                          ↓
┌─────────────────────────────────────────────────────────────┐
│  Cryptocosm (тёмная материя)                                │
│  ├── Кабины персонажей                                      │
│  └── Архитектура Платформы (метафорически)                  │
└─────────────────────────────────────────────────────────────┘
```

**Результат:** Унифицированная навигация + драматургия в визуале графа.

### Статус

Инструменты созреют в процессе работы. Категории помогут при решении, к чему отнести тот или иной эффект в UI.
