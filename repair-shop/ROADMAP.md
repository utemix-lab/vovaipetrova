...
```
🔀 STRATEGIC FORK — FOUR TRACK MODEL

После P5.0 проект разделён на 4 независимых трека:
- TRACK 1 — ONTOLOGY (Universe)      ← АКТИВЕН
- TRACK 2 — PLATFORM (MeaningEngine) ← Стабилен (v0.5.0)
- TRACK 3 — EPISTEMIC OPERATORS      ← Ожидает
- TRACK 4 — PRESENTATION (UI)        ← Стабилен

Всего: 808 тестов (152 engine + 656 render).

См. раздел "STRATEGIC FORK — FOUR TRACK MODEL" ниже.
```
...
После завершения P5.0 Separation система стабилизирована как:

```
Platform (MeaningEngine)
        ↑
World as Plugin (vovaipetrova)
        ↑
UI (render)
```

На данном этапе стало очевидно, что **дальнейшее развитие не является линейным**.

Проект разделяется на **четыре независимых трека разработки**.

```
Это не этапы.
Это параллельные направления.
```

К этой развилке необходимо регулярно возвращаться, чтобы не смешивать уровни.
...
│   ONTOLOGY             PLATFORM             EPISTEMIC            PRESENTATION│
│   (Universe)           (MeaningEngine)      OPERATORS            (UI)       │
│                                             (Инструменты)                   │
│   ┌─────────┐          ┌─────────┐          ┌─────────┐          ┌─────────┐│
│   │ Domain  │          │Structural│         │Operational│        │Presentation│
│   │ Layer   │          │ Layer   │          │ Layer   │          │ Layer   ││
│   └─────────┘          └─────────┘          └─────────┘          └─────────┘│
│                                                                             │
│   Типы узлов           WorldInterface       Pointer-tags         Подсветки │
│   Персонажи            Schema               Протоколы            Виджеты   │
│   Границы              WorldAdapter         Линзы                Навигация │
│   Завершённость        Контракты            Генеративные         Режимы    │
│   Навигация            Multi-world          операции             Universe/ │
│                                                                  Cryptocosm│
└─────────────────────────────────────────────────────────────────────────────┘
```
...
#### TRACK 1 — ONTOLOGY (Universe)

**Назначение:** Определение структуры мира.

**Сюда входит:**
- Типы узлов (области знания)
- Персонажи как линзы
- Границы онтологии
...
- Платформа
- LLM
- Инструменты обработки знаний
- UI-режимы

**Это чистый domain layer.**

---

#### TRACK 2 — PLATFORM (MeaningEngine)

**Назначение:** Гарантировать структурную целостность.

**Сюда входит:**
- WorldInterface
- Schema
- WorldAdapter
- Контракты подключения мира
- Multi-world архитектура
...
**Это structural layer.**

---

#### TRACK 3 — EPISTEMIC OPERATORS (Инструменты)

**Назначение:** Работа с внешними знаниями через мир.

**Сюда входит:**
- Pointer-tags
- Протоколы запроса
- Форматы вывода
- Линзы (режиссура, брендинг, обзор, разбор терминов и т.д.)
- Генеративные операции

**Не входит:**
- Онтология мира
- Контракты платформы
- UI-представление архитектуры

**Это operational layer.**

---

#### TRACK 4 — PRESENTATION (UI / Projection)

**Назначение:** Визуализация структуры.

**Сюда входит:**
- Подсветки
- Виджеты
- Навигация
- Режимы (Visitor / Developer / Client)
- Universe vs Cryptocosm как два источника данных

**Важно:**
```
Cryptocosm — это не meta-онтология.
Это альтернативный набор узлов, отображаемый тем же UI-движком.
Никакого смешения с Universe.
```
...
**Треки не знают друг о друге напрямую.**

Связи допускаются только через определённые границы:

| Связь | Механизм |
|-------|----------|
| Platform ↔ World | через контракт (WorldInterface) |
| UI ↔ данные | через renderer |
| Operators ↔ World | через pointer-протоколы |

**Запрещено:**
- Онтология, влияющая на платформу
- UI, влияющий на Engine
- Cryptocosm, влияющий на Universe
...
```
Проект перестал быть линейным roadmap.
Теперь это система координат.
```

Любая новая идея должна сначала отвечать:

> **К какому треку она относится?**

Если идея касается нескольких треков — её нужно **декомпозировать**.

---

### 🚦 Текущее решение

Мы начинаем движение по:

**TRACK 1 — ONTOLOGY (Universe)**

**Цель:**
Строго определить, что такое Мир,
без метафор,
без LLM,
без платформенных расширений.
...
без стабилизации онтологии —

```
возникнет ощущение зыбкости
и начнётся смешение уровней.
```

---

### 🧠 Это новый тип развилки
...
**Эта развилка — концептуальная.**

Она определяет, как проект будет мыслиться дальше.

К ней нужно возвращаться регулярно.
...
```
После P5.0 проект разделён на 4 независимых трека.
Дальнейшая работа ведётся осознанно в пределах выбранного трека.
Сейчас активен Track 1 — Ontology.
```

| Трек | Слой | Статус |
|------|------|--------|
| TRACK 1 — ONTOLOGY | Domain | **АКТИВЕН** |
| TRACK 2 — PLATFORM | Structural | Стабилен (v0.5.0) |
| TRACK 3 — EPISTEMIC OPERATORS | Operational | Ожидает |
| TRACK 4 — PRESENTATION | Presentation | Стабилен |
