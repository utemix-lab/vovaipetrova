...
**Контекстное меню узла — открытый вопрос:**

При клике на узел окно System уже показывает все три слоя I/S/P. Контекстное меню с теми же пунктами — дублирование.

Возможные действия для ПКМ по узлу (требуют отдельного рассмотрения):
- **Скопировать ссылку** — получить URL/ID узла для шаринга
- **Показать связи** — визуальный эффект подсветки на графе
- **Закрепить** — зафиксировать узел в фокусе
- **Сравнить** — добавить узел в режим сравнения (механика не определена)

**Альтернатива:** Эти действия могут быть реализованы в окне Service (окно действий), а не через контекстное меню.

**Статус:** Открытый вопрос, требует отдельного обсуждения.

### Различие: I (Информация) vs Story (Легенда)

| Элемент | Вопрос | Тип нарратива |
|---------|--------|---------------|
| **I — Информация** | "Что это значит?" | Системный нарратив (документация) |
| **Story (окно)** | "Какая история?" | Художественный нарратив, легенда, игровой контекст |

**I ≠ Story.** Информация объясняет сущность, Story рассказывает историю.

### Четыре окна — полная картина Системы
...
│                                                                     │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐ │
│  │   STORY     │  │   SYSTEM    │  │   SERVICE   │  │   STORAGE   │ │
│  │             │  │             │  │             │  │             │ │
│  │ Легенда     │  │ I, S, P     │  │ Операторы   │  │ Графы       │ │
│  │ Художеств.  │  │ Информация  │  │ Действия    │  │ Данные      │ │
│  │ нарратив    │  │ о сущности  │  │ над данными │  │ Проекции    │ │
│  └─────────────┘  └─────────────┘  └─────────────┘  └─────────────┘ │
│                                                                     │
│  ЧТО?             ЧТО?            КАК?            ГДЕ?              │
│  (история)        (понимание)     (работа)        (хранение)        │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

### Эпистемические Операторы — где они живут?

**Операторы — это не слой информации, а механизм работы с данными.**

| Оператор | Что делает | Пример |
|----------|------------|--------|
| **PROJECT** | Показать только часть данных | "Покажи только плагины категории EQ" |
| **FILTER** | Отсеять по условию | "Убери всё, что дороже $100" |
| **EXPAND** | Раскрыть связи | "Покажи все плагины, которые использует этот эффект" |
| **INTERSECT** | Найти общее | "Что общего между двумя воркбенчами?" |

**Различие:**

```
I, S, P     — ЧТО показывать (слои информации)
Операторы  — КАК работать с данными (механизм)
```

**Операторы живут в Service (окно действий), не в System (окно информации).**

Три полоски в Service — вход в чат/действия, где операторы доступны через естественный язык или кнопки.

**Различие запросов и операторов:**

```
Запрос к графу:     "Дай мне узел X"           → получить данные
Оператор:           "Покажи связи узла X"      → трансформировать данные
```

**Статус:** Концепция определена, требует визуального языка для понимания.
...
## Карта состояния Системы (22.02.2026)
...
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  СТРУКТУРА (✅ сделано)          СОДЕРЖАНИЕ (❌ пусто)              │
│  ─────────────────────           ──────────────────────             │
│  • Узлы графа                    • Описания узлов                   │
│  • Связи между узлами            • Атрибуты и теги                  │
│  • Типы (character, domain...)   • catalogRefs                      │
│  • Навигация по графу            • Данные в каталогах               │
│  • Визуальные подсказки                                             │
│  • Окна и их поведение                                              │
│  • Операторы в Engine                                               │
│                                                                     │
│  МЫ ПОСТРОИЛИ ДОМ, НО НЕ ЗАСЕЛИЛИ ЕГО                              │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```
...
| Слой | Что есть | Что даёт | Статус |
|------|----------|----------|--------|
| **Структура** | Узлы, связи, типы | Навигация, переходы | ✅ Сделано |
| **Содержание** | Описания, атрибуты, каталоги | Смысл, понимание | ❌ Пусто |
...
Узел "Vova" должен содержать:
  { 
    id: "vova", 
    type: "character", 
    label: "Vova",
    description: "Музыкант, продюсер, работает с VST",
    tags: ["production", "mixing", "synth"],
    catalogRefs: { vst: ["serum", "massive"] }
  }
```

**Когда узлы наполнены → операторы работают → LLM понимает смысл.**

---

## Четыре окна — полная картина

| Окно | Вопрос | Содержимое | Компонент |
|------|--------|------------|-----------|
| **Story** | "Какая история?" | Художественный нарратив, легенда | World |
| **System** | "Что это?" | I/S/P — информация, спецификация, проекция | World + Engine + Render |
| **Service** | "Что делать?" | Действия, операторы, чат | Engine (операторы) |
| **Storage** | "Где данные?" | Графы, каталоги, проекции | World + Render |

### I/S/P вкладки окна System

| Вкладка | Вопрос | Источник | Компонент |
|---------|--------|----------|-----------|
| **I — Информация** | "Что это значит?" | story.json, описания | World |
| **S — Спецификация** | "Как это устроено?" | schema.json, контракты | Engine |
| **P — Проекция** | "Как это показано?" | ui-config, visual-spec | Render |

**I ≠ Story:** Информация объясняет сущность, Story рассказывает историю.

---

## Операторы — эволюционный подход

### Что такое оператор?

```
Оператор = функция Платформы, которая трансформирует данные графа

Вход:   Данные графа + параметры
Выход:  Трансформированные данные (не меняет граф)
```

### Операторы уже реализованы в Engine

| Оператор | Что делает | Статус |
|----------|------------|--------|
| **PROJECT** | Спроецировать каталог через узел | ✅ 194 теста |
| **FILTER** | Отфильтровать по условию | ✅ Реализован |
| **EXPAND** | Раскрыть связи на глубину | ✅ Реализован |
| **INTERSECT** | Найти пересечение | ✅ Реализован |

### Почему операторы не работают?

```
Операторы работают, но им нечего обрабатывать.
Узлы пусты → PROJECT("vova", "vst") возвращает []
Нужны данные в узлах (catalogRefs, tags).
```

### Эволюция: UI-логика → Операторы

```
Этап 1 (сейчас):
  UI-логика в Render (Query Mode, кнопки)
  Работает, но захардкожено

Этап 2 (скоро):
  Видим паттерны → выносим в Engine
  Query Mode → FILTER
  "Показать связи" → EXPAND

Этап 3 (потом):
  UI вызывает Engine
  LLM вызывает те же операторы
  Единый механизм для всех
```

**Query Mode — прообраз оператора FILTER.**

### Что дают операторы?

| Для кого | Без операторов | С операторами |
|----------|----------------|---------------|
| **UI** | Логика захардкожена | Вызывает Engine |
| **LLM** | Может только говорить | Может действовать |
| **Тесты** | Тестируем UI | Тестируем Engine отдельно |
| **Другой разработчик** | Пишет свою логику | Использует готовые операторы |

### LLM: от консультанта к исполнителю

```
Без операторов:
  Пользователь: "Какие плагины использует и Vova, и Petrova?"
  LLM: "Чтобы это узнать, откройте воркбенч Vova, затем..."

С операторами:
  Пользователь: "Какие плагины использует и Vova, и Petrova?"
  LLM: *вызывает INTERSECT* → "Оба используют: Serum, FabFilter Pro-Q..."
```

**Операторы превращают LLM из консультанта в исполнителя.**

---

## Разведение понятий по Трекам

| Понятие | Track | Кто отвечает |
|---------|-------|--------------|
| Узлы, связи, типы | Track 1 (Ontology) | World |
| Схема, валидация | Track 2 (Schema) | Engine |
| Операторы, каталоги | Track 3 (Operators) | Engine |
| UI, окна, проекции | Track 4 (Projection) | Render |
| LLM-агент, автоматизация | Track 5 (Agency) | Engine + World |
| Визуальные стеки | Track 6 (Stacks) | Render |

### Что кому даёт

| Роль | Что видит | Что делает |
|------|-----------|------------|
| **Автор (ты)** | Треки, архитектуру, код | Строит Систему |
| **Пользователь** | UI, окна, навигацию | Исследует Мир |
| **LLM** | Операторы, контекст | Помогает, действует |

---

## Следующие шаги (приоритет)

| # | Задача | Track | Блокирует |
|---|--------|-------|-----------|
| 1 | **Наполнить узлы данными** (seed.json) | Track 1 | Операторы, LLM |
| 2 | Связать узлы с каталогами (catalogRefs) | Track 1 + 3 | PROJECT |
| 3 | VST-граф в Storage | Track 4 + 6 | Визуализация |
| 4 | Контекстное меню ПКМ для кнопок UI | Track 4 | Мета-документация |
| 5 | Камера на VSTablishment | Track 6 | — |

### Критерий проверки состояния

```
✅ Структура готова, если:
   - Все типы узлов определены в schema.json
   - Навигация работает
   - Окна отображаются

✅ Содержание готово, если:
   - Узлы имеют description, tags, catalogRefs
   - engine.project(nodeId, catalogId) возвращает данные
   - LLM может ответить на вопрос о конкретном узле

✅ Операторы работают, если:
   - UI вызывает Engine, а не локальную логику
   - LLM может вызвать тот же оператор
   - Результат одинаков для UI и LLM
```

---

## Визуальный язык — открытый вопрос

Абстракции (операторы, треки) требуют визуального языка для понимания:

| Подход | Что визуализирует | Статус |
|--------|-------------------|--------|
| Диаграмма потоков | Как данные движутся между окнами | ❓ |
| Анимация операторов | Как FILTER/EXPAND меняют граф | ❓ |
| Интерактивный туториал | Пользователь сам применяет операторы | ❓ |

**Статус:** Концепция определена, визуальный язык требует отдельного исследования.
