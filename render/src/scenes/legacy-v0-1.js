...
function getNodeColor(node) {
  const base = palette.type[node.type] || palette.status[node.status] || palette.nodeDefault;
...
function createLinkObject(link) {
  ...
  const spikeMaterialA = new THREE.MeshBasicMaterial({ color: 0xffffff, transparent: true });
  const spikeMaterialB = new THREE.MeshBasicMaterial({ color: 0xffffff, transparent: true });
  const spikeA = new THREE.Mesh(spikeGeometry, spikeMaterialA);
  const spikeB = new THREE.Mesh(spikeGeometry, spikeMaterialB);

  const group = new THREE.Group();
  group.add(line);
  group.add(spikeA);
  group.add(spikeB);
  group.userData = { line, spikeA, spikeB };

  return group;
}

function updateLinkObject(obj, position, link) {
  const { line, spikeA, spikeB } = obj.userData;
  const start = warpPosition(new THREE.Vector3(position.start.x, position.start.y, position.start.z));
  const end = warpPosition(new THREE.Vector3(position.end.x, position.end.y, position.end.z));
  ...
  spikeA.position.copy(startBase).add(unitDir.clone().multiplyScalar(spikeLengthStart / 2));
  spikeB.position.copy(endBase).add(unitDir.clone().multiplyScalar(-spikeLengthEnd / 2));

  spikeA.material.color.copy(startColor);
  spikeB.material.color.copy(endColor);
  spikeA.material.opacity = isHighlighted ? 0.9 : 0.7;
  spikeB.material.opacity = isHighlighted ? 0.9 : 0.7;
}
